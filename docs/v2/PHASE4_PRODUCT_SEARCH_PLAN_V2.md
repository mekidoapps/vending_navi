...
Phase 4では固定半径を導入せず、
現在表示中のGoogle Map viewportを検索スコープとして採用した。

50m / 100m / 300m / 500m等の固定値は、
Phase 4だけでは決定根拠が不足している。

実データ量とクローズドテストの利用行動を見て、
最大取得件数・範囲拡張UXと合わせて後続で決定する。

## 5. OI-004 情報古さ

未確定を維持する。

P4-07までに検索対象商品の優先表示は成立したが、
具体的な更新頻度データはまだ不足している。

Phase 4では`確認済み` / `あるかも`を利用し、
`以前の情報`へ切り替える具体日数は設定しない。

...
void main() {
  testWidgets(
    'machine report keeps success and blocks the same target in actor mode',
    (tester) async {
      ...
      expect(gateway.changes, <_Target>[
        _Target('machine', _machineId, null, null),
      ]);
      expect(harness.completed, 1);
      expect(_result(harness.container), isNotNull);
      expect(
        harness.container.read(blockedContentProvider).machineIds,
        <String>{_machineId},
      );
    },
  );
  ...
  testWidgets(
    'machine cancel does not call block and keeps report success unchanged',
    (tester) async {
      final gateway = _Gateway(mode: ContentBlockMode.actor);
      final harness = await _pump(tester, gateway: gateway, draft: _draft());
  ...
  testWidgets(
    'machine block failure does not turn report success into an error',
    (tester) async {
      ...
      expect(find.text('報告を受け付けました。非表示設定を変更できませんでした。'), findsOneWidget);
      expect(harness.completed, 1);
      expect(_result(harness.container), isNotNull);
      expect(
        harness.container.read(machineReportControllerProvider).failure,
        isNull,
      );
    },
  );
  ...
  testWidgets(
    'photo report preserves its exact target for actor and content modes',
    (tester) async {
      ...
      for (final mode in ContentBlockMode.values) {
        ...
        expect(gateway.changes, <_Target>[
          _Target('photo', _machineId, photoId, null),
        ]);
        expect(harness.completed, 1);
        expect(_result(harness.container), isNotNull);
        harness.dispose();
  ...
  testWidgets('photo block failure retains the successful report', (
    tester,
  ) async {
  ...
  testWidgets(
    'product report preserves its exact target for actor and content modes',
    (tester) async {
      ...
      for (final mode in ContentBlockMode.values) {
        ...
        expect(gateway.changes, <_Target>[
          _Target('product', _machineId, null, productId),
        ]);
        expect(harness.completed, 1);
        expect(_result(harness.container), isNotNull);
        harness.dispose();
  ...
  testWidgets(
    'product block failure and mode lookup failure keep report success private',
    (tester) async {
      final failureGateway = _Gateway(
...
final class _SuccessRepository implements MachineReportRepository {
  ...
  Future<AppResult<MachineReportResult>> submitReport({
    ...
    MachineReportResult(
      machineId: draft.machineId,
      reportId: 'r_0123456789abcdef0123456789abcd',
    ),
  );
}

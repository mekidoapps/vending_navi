...
void main() {
  testWidgets(
    'P10-I03A 写真取得からAI候補を確認して正式登録後に詳細へ進む',
    (WidgetTester tester) async {
      ...
      final draftAfterUpload =
          container.read(machineRegistrationControllerProvider).draft;
      ...
      expect(
        container
            .read(registrationPhotoRecognitionControllerProvider)
            .stage,
      ...
      await tester.scrollUntilVisible(
        find.byKey(
          const Key('registrationPhotoCandidatesSaveButton'),
        ),
        300,
      );
      ...
      final confirmed =
          container.read(machineRegistrationControllerProvider);

      expect(confirmed.step, MachineRegistrationStep.confirm);
      expect(
        confirmed.draft.manufacturerId?.value,
        'suntory',
      );
      expect(
        confirmed.draft.confirmedProductIds
            .map((item) => item.value),
        contains('suntory_boss_black'),
      );

      expect(find.text('登録内容の確認'), findsOneWidget);
      expect(find.byKey(const Key('registrationConfirmationProductSummary')), findsOneWidget);

      //
      // Final confirmation -> create -> detail.
      //
      await _submitAndExpectDetail(
        tester,
  ...
  testWidgets(
    'P10-I03B AI認識失敗後にメーカー登録へ切り替えて正式登録できる',
    (WidgetTester tester) async {
      final registrationRepository = _FakeMachineRegistrationRepository();
      final recognitionRepository = _FakePhotoRecognitionRepository(
      ...
      expect(
        find.text('写真から候補を見つけられませんでした'),
        findsOneWidget,
      );
      expect(
        find.text('メーカーから登録する'),
        findsOneWidget,
      );
      ...
      expect(
        find.byKey(
          const Key('registrationManufacturer_suntory'),
        ),
        findsOneWidget,
      );
      ...
      expect(
        fallbackState.draft.manufacturerId?.value,
        'suntory',
      );
  ...
  return createAppRouter(
    ...
    machineDetailBuilder: (_, machineId) {
      return Scaffold(
        ...
        body: Center(
          child: Text(
            'photo-created-detail:${machineId.value}',
          ),
        ),
      );
    },
...
Future<void> _submitAndExpectDetail(
  ...
  expect(
    find.byKey(
      const Key('p10PhotoCreatedMachineDetail'),
    ),
    findsOneWidget,
  );
  ...
  expect(
    find.text(
      'photo-created-detail:machine_p10_photo_created',
    ),
    findsOneWidget,
  );
}
...
Manufacturer _manufacturer() {
  return Manufacturer(
    ...
    presetProductIds: <ProductId>[
      ProductId.parse('suntory_boss_black'),
    ],
...
List<Product> _products() {
  return ProductMasterFixture.products
      .where(
        (product) =>
            product.id.value == 'suntory_boss_black',
      )
      .toList(growable: false);
}

final class _FakePhotoCaptureSource
...
final class _FakePhotoNormalizer
    implements RegistrationPhotoNormalizer {
  const _FakePhotoNormalizer();

  @override
  Future<NormalizedRegistrationPhoto> normalize(
...
final class _FakeTemporaryPhotoUploader
  ...
  Future<TemporaryRegistrationPhotoUpload> upload({
    ...
    return TemporaryRegistrationPhotoUpload(
      uploadId: uploadId,
      objectPath:
          'machine_uploads/p10/$uploadId.jpg',
    );
  }
}
...
final class _FakePhotoRecognitionRepository
  ...
  _FakePhotoRecognitionRepository({
  ...
  createVendingMachine(
    ...
    return AppResult<MachineRegistrationResult>.success(
      MachineRegistrationResult(
        machineId: VendingMachineId.parse(
          'machine_p10_photo_created',
        ),
        created: true,
      ),
    );
...
final class _FakeProductRepository
  ...
  @override
  Future<AppResult<Product>> getProduct(
    ProductId id,
  ) async {

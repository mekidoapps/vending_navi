...
void main() {
  testWidgets('P10-I01 商品検索から固定カードを経由して詳細へ検索状態を引き継ぐ', (
    WidgetTester tester,
  ) async {
    ...
    final router = createAppRouter(
      ...
      v2Builder: (_) => V2HomeMapScreen(
        autoLocate: false,
        mapBuilder: (_) =>
            const ColoredBox(key: Key('p10FakeMap'), color: Colors.white),
      ),
    ...
    final matchLabel = find.byKey(const Key('selectedMachineSearchMatchLabel'));

    expect(matchLabel, findsOneWidget);
    expect(tester.widget<Text>(matchLabel).data, 'BOSS ブラック');

    await tester.tap(find.byKey(const Key('selectedMachineDetailButton')));
    await tester.pumpAndSettle();

    expect(find.byKey(const Key('p10MachineDetail')), findsOneWidget);

    expect(
      find.text('detail:machine_p10_search:suntory_boss_black'),
      findsOneWidget,
    );

    expect(
      container.read(productSearchSelectionControllerProvider)?.id.value,
      'suntory_boss_black',
    );
  });
}
...
final class _FakeIndexRepository implements MachineProductIndexRepository {
  _FakeIndexRepository(this.machineId, this.expectedProductId);

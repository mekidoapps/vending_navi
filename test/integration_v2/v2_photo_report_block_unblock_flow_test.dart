...
const machineId = 'machine-a',
    photoId = 'photo-a',
    handle = 'safe_handle_never_render';
final machine = VendingMachine(
  id: VendingMachineId.parse(machineId),
  schemaVersion: 2,
  name: 'テスト自販機',
...
void main() {
  testWidgets(
    'formal photo report to block, MyPage unblock, then detail restore',
    (tester) async {
      ...
      final c = ProviderContainer(
        overrides: [
          ...
          userProfileRepositoryProvider.overrideWithValue(_Profiles()),
          blockedContentGatewayProvider.overrideWithValue(blocks),
          machineReportRepositoryProvider.overrideWithValue(reports),
          vendingMachineDetailProvider(machine.id).overrideWithValue(
            AsyncData(AppResult<VendingMachineDetailData>.success(_detail())),
          ),
      ...
      final r = GoRouter(
        initialLocation: '/v2/machines/$machineId',
        routes: [
          GoRoute(
            name: AppRoute.v2MachineDetail.name,
            path: AppRoute.v2MachineDetail.path,
            builder: (_, __) =>
                V2VendingMachineDetailScreen(machineId: machine.id),
          ),
          GoRoute(
            name: AppRoute.v2MachineReport.name,
            path: AppRoute.v2MachineReport.path,
            builder: (x, s) => V2MachineReportScreen(
              machineId: machine.id,
              photoId: s.uri.queryParameters['photoId'],
              onReviewPressed: () => x.pushNamed(
          ...
          GoRoute(
            ...
            builder: (x, _) => V2MachineReportConfirmationScreen(
              machineId: machine.id,
              onCompleted: () {
                c.read(machineReportControllerProvider.notifier).reset();
                x.goNamed(
          ...
          GoRoute(
            ...
            builder: (_, __) => const V2MyPageScreen(),
          ),
        ],
      );
      ...
      expect(
        photo.displayReferenceFor(machineId),
        'vending_machines/machine-a/photo-a/original.jpg',
      );
      expect(find.byKey(const Key('formalPhoto_photo-a')), findsOneWidget);

      await tester.tap(find.byKey(const Key('photoActions_photo-a')));
      await tester.pumpAndSettle();
      await tester.tap(find.text('この写真を報告'));
      await tester.pumpAndSettle();
      await tester.tap(
        find.byKey(const Key('machineReportCategory_inappropriatePhoto')),
      );
      final reviewButton = find.byKey(const Key('machineReportReviewButton'));
      final reportScrollable = find
          .descendant(
            of: find.byKey(const Key('machineReportScreen')),
      ...
      await tester.pumpAndSettle();
      expect(reports.draft?.targetType, 'photo');
      expect(reports.draft?.machineId.value, machineId);
      expect(reports.draft?.photoId, photoId);
      expect(find.text('この写真も非表示にしますか？'), findsOneWidget);
      await tester.tap(find.byKey(const Key('reportBlockConfirmButton')));
      await tester.pumpAndSettle();
      expect(blocks.blocked, isTrue);
      expect(find.byKey(const Key('formalPhoto_photo-a')), findsNothing);
      expect(find.text('テスト自販機'), findsOneWidget);
      expect(find.text('商品A'), findsOneWidget);

      r.goNamed(AppRoute.v2MyPage.name);
      await tester.pumpAndSettle();
      expect(find.text('写真'), findsOneWidget);
      expect(find.text(handle), findsNothing);
      expect(find.text('actor_uid_never_render'), findsNothing);
      expect(find.text('private/path/never_render'), findsNothing);
      await tester.tap(find.text('解除'));
      await tester.pumpAndSettle();
      expect(blocks.unblocked, <String>[handle]);
...
VendingMachineDetailData _detail() => VendingMachineDetailData(
  machine: machine,
...
final class _Blocks implements BlockedContentGateway {
  ...
  Future<void> change({
    ...
    if (operation == 'blockContentSource') {
      expect(data, {
        'targetType': 'photo',
        'machineId': machineId,
        'photoId': photoId,
      ...
      unblocked.add(data['handle']! as String);
      blocked = false;
    }
  }
  ...
  Future<Object?> getBlockedContentIds() async => {
    'machineIds': <String>[],
    'photoIds': blocked ? <String>[photoId] : <String>[],
    'productIds': <String>[],
    'blocks': blocked
        ? <Map<String, Object?>>[
            {
...
final class _Reports implements MachineReportRepository {
  ...
  Future<AppResult<MachineReportResult>> submitReport({
    ...
    return AppResult.success(
      MachineReportResult(machineId: draft.machineId, reportId: 'report-a'),
    );
  }
}

final class _Auth implements AuthRepository {
  final s = AuthenticatedAuthSession(
    AuthUser(
      uid: 'actor_uid_never_render',
      email: null,
      displayName: 'display_name_never_render',
      providerIds: ['password'],
      emailVerified: true,
    ),
  );
  @override
  AuthSession get currentSession => s;
  @override
  Stream<AuthSession> watchSession() => Stream.value(s);
  @override
...
final class _Profiles implements UserProfileRepository {
  @override
  Future<AppResult<UserProfile>> getOrCreateProfile({
    required String uid,
  }) async => AppResult.success(UserProfile(uid: uid));
  @override
